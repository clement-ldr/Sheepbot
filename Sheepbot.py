...
######PORN
# 8=====D
with open('pornGif.txt', 'r') as pronF:
    pronGif = pronF.read()
pronGif = pronGif.split('\n')

with open('cumsluts.txt', 'r') as cum:
    cumslut = cum.read()
cumslut = cumslut.split('\n')

a = open('pics_url.txt','r').read()
porn_pics = eval(a)

g = open('povid.txt','r').read()
porn_vid = eval(g)
...
class NSFW:
    ...
    def vido(self,query):
        r = requests.get('https://www.sex.com/search/videos?query='+query+'&page='+str(random.randint(0,5)))
        r = r.text
        soup = BeautifulSoup(r)
        #print(soup)
        tag = soup.find_all('a',"image_wrapper")
        if len(tag)<1:
            r = requests.get('https://www.sex.com/search/videos?query='+query)
            r = r.text
            soup = BeautifulSoup(r)
            #print(soup)
            tag = soup.find_all('a',"image_wrapper")
        i = random.choice(tag)
        e = 'https://www.sex.com'+i['href']
        ri = requests.get(e)
        ri = ri.text
        soupi = BeautifulSoup(ri)
        tog = soupi.find_all('script')
        a = reg.findall(str(tog))
        fine = requests.head('http://sex.com'+a[0], allow_redirects=True)
        return fine.url




    @commands.command(pass_context=True, no_pm=True, hide=True, aliases=['NSFW'])
    async def nsfw(self, ctx):
        """Add/Remove the channel NSFW tag"""
        perms = ctx.message.author.server_permissions
        if not perms.administrator:
            await self.bot.say(':fire::no_entry_sign: You need to be admin! :no_entry_sign::fire:')
            return
        if permi['NSFW'].get_serv(ctx.message.server.id) == None:
            permi['NSFW'].add_server(str(ctx.message.server.id))
            
        if permi['NSFW'].get_serv(ctx.message.server.id) != None:
            if ctx.message.channel.id in permi['NSFW'].get_serv(ctx.message.server.id)['NSFW']:
                ty = permi['NSFW'].remove_NSFW(ctx.message.server.id,ctx.message.channel.id)
                if ty:
                    await self.bot.say("NSFW disable for this channel")
                else:
                    await self.bot.say("ERROR")
            else:
                ty = permi['NSFW'].add_NSFW(ctx.message.server.id,ctx.message.channel.id)
                if ty:
                    await self.bot.say("NSFW enable for this channel, have fun !")
                else:
                    await self.bot.say("ERROR")

        

    @commands.command(pass_context=True, no_pm=True, aliases=['R34','r34','Rule34','34'])
    async def rule34(self, ctx):
        """The Rule34"""
        #if sp.get_server(ctx.message.server.id)["nsfw"]:
        if sp.get_server(ctx.message.server.id)["nsfw"]:
            if is_permi('NSFW',ctx.message.server.id,ctx.message.channel.id):
                try:
                    prefix = ctx.prefix
                    args = ctx.message.content.replace('{0}rule34 '.format(prefix),'')
                    args = args.replace('{0}r34 '.format(prefix),'')
                    args = args.replace('{0}Rule34 '.format(prefix),'')
                    args = args.replace('{0}R34 '.format(prefix),'')
                    args = args.replace('{0}34 '.format(prefix),'')
                    tags = args.replace(' ','+')
                    ...
                    async with aiohttp.ClientSession(connector=conn) as session:
                        async with session.get(r34_url) as resp:
                            retu = await resp.read()
                    posts = html.fromstring(retu)
                    choice = random.choice(posts)
                    url = str(choice.attrib['file_url'])
                except Exception as e:
                    logging.warning(str(e))
                    print(e)
                try:
                    ...
                    embed.set_footer(text='Remember to replace spaces in tags with an underscore, as a space separates multiple tags')
                    await bot.say(embed=embed)
            else:
                await self.bot.say("NSFW is disable here, enable with !!NSFW")
        else:
            await self.bot.say("NSFW is disabled for your server, enable it on the website ( http://sheepbot.net )")



    @commands.command(pass_context=True, no_pm=True, aliases=['sein','nichon','bb','boob'])
        ...
        if sp.get_server(ctx.message.server.id)["nsfw"]:
            if is_permi('NSFW',ctx.message.server.id,ctx.message.channel.id):
                try:
                    ...
                    number = random.randint(1, 11240)
                    url_api = api_base + str(number)
                    async with aiohttp.ClientSession() as session:
                            async with session.get(url_api) as resp:
                                fin = await resp.json()
                    fin = fin[0]
                    image_url = 'http://media.oboobs.ru/' + fin['preview']
                    embed = discord.Embed(color=0x9933FF)
            ...
            await self.bot.say("NSFW is disabled for your server, enable on the website ( http://sheepbot.net )")



    @commands.command(pass_context=True, no_pm=True, aliases=['buts','ass','butt','rump','fanny'])
        ...
        if sp.get_server(ctx.message.server.id)["nsfw"]:
            if is_permi('NSFW',ctx.message.server.id,ctx.message.channel.id):
                try:
                    ...
                    number = random.randint(1, 4300)
                    url_api = api_base + str(number)
                    async with aiohttp.ClientSession() as session:
                            async with session.get(url_api) as resp:
                                fin = await resp.json()
                    fin = fin[0]
                    image_url = 'http://media.obutts.ru/' + fin['preview']
                    embed = discord.Embed(color=0x9933FF)
                    embed.set_image(url=image_url)
                    await self.bot.say(embed=embed)
                except Exception as e:
    ...
    @commands.command(pass_context=True, no_pm=True, aliases=['porn_vid','pvid','PV','VideoPorn','Porno'])
    async def porn_video(self, ctx, *, query:str=None):
        """Search porn Gifs ;)"""
        if sp.get_server(ctx.message.server.id)["nsfw"]:
            if is_permi('NSFW',ctx.message.server.id,ctx.message.channel.id):
                try:
                    if query == None:
                        merde = random.choice(porn_vid)
                    else:
                        query = query.replace(' ','+')
                        #query = 'swallow+cum'                        
                        loop = asyncio.get_event_loop()
                        merde = loop.run_in_executor(None,self.vido,query)
                        await merde
                        merde = merde.result()
                except Exception as e:
                    raise e
                    await self.bot.say('No result')
                await self.bot.say(merde)
            else:
                await self.bot.say("NSFW is disable here, enable with !!NSFW")
        else:
            await self.bot.say("NSFW is disabled for your server, enable it on the website ( http://sheepbot.net )")
    ...
    @commands.command(pass_context=True, no_pm=True, aliases=['picture_porn','pornpicture','pornimage'])
    async def pics(self, ctx, *, query:str=None):
        """Search porn Gifs ;)"""
        if sp.get_server(ctx.message.server.id)["nsfw"]:
            if is_permi('NSFW',ctx.message.server.id,ctx.message.channel.id):
                try:
                    if query == None:
                        image_url = random.choice(porn_pics)
                    else:
                        query = query.replace(' ','+')
                        reg = re.compile(r'(https?://[-\w;/?:@&=+$\|\_.!~*\|\'()\[\]%#,☺]+[\w/#](\(\))?)(?=$|[\s\',\|\(\).:;?\-\[\]>\)]).jpg')
                        lt = []
                        conn = aiohttp.TCPConnector(
                        family=socket.AF_INET,
                        verify_ssl=False,
                        ...
                        async with aiohttp.ClientSession(connector=conn) as session:
                            async with session.get('http://www.sex.com/search/pics?query='+query+'&page='+str(random.randint(0,10))) as resp:
                                r = await resp.text()
                        e = reg.findall(r)
                        if len(e)<10:
                            async with aiohttp.ClientSession(connector=conn) as session:
                                async with session.get('http://www.sex.com/search/pics?query='+query) as resp:
                                    r = await resp.text()
                        e = reg.findall(r)
                        for z in e:
                            lt.append(z[0]+'.jpg\n')
                        image_url = random.choice(lt)
                except:
                    await self.bot.say('No result')
                embed = discord.Embed(color=0x9933FF)
                embed.set_image(url=image_url)
    ...
    @commands.command(pass_context=True, no_pm=True, aliases=['pgif','PornG','PG','pornG','gifporn'])
    async def porn_gif(self, ctx, *, query:str=None):
        """Search porn Gifs ;)"""
        if sp.get_server(ctx.message.server.id)["nsfw"]:
            if is_permi('NSFW',ctx.message.server.id,ctx.message.channel.id):
                try:
                    if query == None:
                        image_url = random.choice(pronGif)
                    else:
                        query = query.replace(' ','+')
                        reg = re.compile(r'(https?://[-\w;/?:@&=+$\|\_.!~*\|\'()\[\]%#,☺]+[\w/#](\(\))?)(?=$|[\s\',\|\(\).:;?\-\[\]>\)]).gif')
                        lt = []
                        conn = aiohttp.TCPConnector(
                        family=socket.AF_INET,
                        verify_ssl=False,
                        ...
                        async with aiohttp.ClientSession(connector=conn) as session:
                            async with session.get('http://www.sex.com/search/gifs?query='+query+'&page='+str(random.randint(0,3))) as resp:
                                r = await resp.text()
                        e = reg.findall(r)
                        if len(e)<10:
                            async with aiohttp.ClientSession(connector=conn) as session:
                                async with session.get('http://www.sex.com/search/gifs?query='+query) as resp:
                                    r = await resp.text()
                        e = reg.findall(r)
                        for z in e:
                            if not z[0]+'.gif' == 'https://d5nxst8fruw4z.cloudfront.net/atrk.gif':
                                lt.append(z[0]+'.gif\n')
                        image_url = random.choice(lt)
                except:
                    await self.bot.say('No result')
    ...
    async def neko(self, ctx):
        ...
        if sp.get_server(ctx.message.server.id)["nsfw"]:
            if is_permi('NSFW',ctx.message.server.id,ctx.message.channel.id):
                try:
                    args = ctx.message.content.replace('!!yande','')
                    ...
                    image_url = data['url'] or data['neko']
                    embed = discord.Embed(color=0x9933FF)
                    embed.set_image(url=image_url)
                    await self.bot.say(embed=embed)
                except Exception as e:
    ...
    @commands.command(pass_context=True, no_pm=True)
    async def yande(self, ctx, args:str =None):
        """NSFW yande"""
        if sp.get_server(ctx.message.server.id)["nsfw"]:
            if is_permi('NSFW',ctx.message.server.id,ctx.message.channel.id):
                try:
